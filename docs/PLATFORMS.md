...
|---|---|---|
| **macOS** (Apple Silicon) | Homebrew bash 5.x | Primary/dev machine. `os/darwin.sh` handles Homebrew, compile flags, openjdk/python kegs, iTerm2. |
...
- Everything else in the shared modules is **guarded**, not duplicated: PATH entries use
  `[ -d ]`/`command -v` checks, tool integrations (pyenv, direnv, cargo, deno) only
